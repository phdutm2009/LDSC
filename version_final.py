...
# if dataset_name == 'dblp' or dataset_name == 'polbooks' or dataset_name == 'dolphins' or dataset_name == 'karate':
#     path = "./datasets/" + dataset_name + ".mtx"   
...
with open(path) as f:
    for line in f:
        ...
        if row:
            for j in row:
                try:
                    temp_array.append(int(j))
                except ValueError:
                    temp_array.append(-1)
            nodes_neighbors[str(i)] = temp_array
            i += 1
N = i
G = nx.Graph()

for node, neighbors in nodes_neighbors.items():
    if neighbors and all(neighbor != -1 for neighbor in neighbors):
        G.add_node(str(node), neighbors=neighbors)
...
def CALCULAT_PERFORMANCE():
    ...
    if NMI_flag == 1:
        ...
        if dataset_name in ('karate', 'dolphins', 'polbooks', 'football', 'Pubmed' , 'Citeseer', 'cora'):
            ...
            G_tmp = nx.Graph()
            G_tmp.add_nodes_from(nodes_temp)
            nodes_temp_set = set(nodes_temp)
            for node in nodes_temp:
                neighbors_node = [neighbor for neighbor in G.neighbors(node) if neighbor in nodes_temp_set]
                for neighbor in neighbors_node:
                    G_tmp.add_edge(node, neighbor)
        ...
        label_set = set(real_labels) | set(detected_labels)

        # Create a confusion matrix between ground truth and detected labels
        mapping_dict = {label: idx for idx, label in enumerate(label_set)}
        conf_matrix = np.zeros((len(mapping_dict), len(mapping_dict)))

        for i in range(len(real_labels)):
            conf_matrix[mapping_dict[real_labels[i]], mapping_dict[detected_labels[i]]] += 1

        # Use the Hungarian algorithm to find the best label mapping
        _, mapping = linear_sum_assignment(-conf_matrix)

        # Map the detected labels to the ground truth labels
        mapped_detected_labels = [mapping[mapping_dict[label]] for label in detected_labels]

        # Calculate F-measure
        f1score_weighted = f1_score(real_labels, mapped_detected_labels, average='weighted')
        print("Weighted F-score is: ", f1score_weighted)
    else:
        detected_labels = predicted_labels()
        print(Fore.GREEN + 'Number of Communities is: ', len(set(detected_labels)), Style.RESET_ALL)
     
    community_dict_module = {node: get_label(int(node)) for node in nodes_temp}

    # Calculate modularity
    modularity_score = c.modularity(community_dict_module, G_tmp)
    print(f'Modularity {dataset_name}: {modularity_score:.4f}')
